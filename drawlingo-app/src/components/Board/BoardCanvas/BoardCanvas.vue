...
const handleMouseDown = (e: MouseEvent): void => {
  ...
  if (roomStore.playerPaitingWord?.id === userStore.id) {
    ...
    >
      <div class="flex relative items-center justify-center w-full h-full">
        <h2 class="text-2xl font-semibold text-white">
          {{
            (roomStore.playerPaitingWord && roomStore.playerPaitingWord.id === userStore.id) ||
      ...
      <BoardToolBar
        v-if="roomStore.playerPaitingWord && roomStore.playerPaitingWord.id === userStore.id"
      ></BoardToolBar>
    </div>
